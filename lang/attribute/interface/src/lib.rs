...
pub fn interface(
    ...
    let method_ixs: Vec<proc_macro2::TokenStream> = item_trait
    .items
    .iter()
    ...
    .map(|method: &syn::TraitItemMethod| {
        ...
        let generate_ix_method_name_s = format!("create_{}_ix", method_name);
        let generate_ix_method_name = generate_ix_method_name_s.as_str();
        quote! {
            pub fn #generate_ix_method_name<'a,'b, 'c, 'info, T: anchor_lang::Accounts<'info> + anchor_lang::ToAccountMetas + anchor_lang::ToAccountInfos<'info>>(
        ...
        .map(|method: &syn::TraitItemMethod| {
            ...
            quote! {
                pub fn #method_name<'a,'b, 'c, 'info, T: anchor_lang::Accounts<'info> + anchor_lang::ToAccountMetas + anchor_lang::ToAccountInfos<'info>>(
                    ...
                    let ix = #generate_ix_method_name(ctx, #(#args),*)?;
